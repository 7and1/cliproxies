...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	if s.accessManager != nil {
		if providers := s.accessManager.Providers(); len(providers) == 0 {
			log.Fatalf("no authentication providers configured; set api-keys or access.providers")
		}
	}
	...
	// Capture allowed CORS origins from config (fallback to localhost for dev)
	s.allowedOrigins = cfg.CORS.AllowedOrigins
	...
	// Register Proxy Grid module
	proxygridModule := proxygrid.NewModule(&cfg.SDKConfig.ProxyGrid)
	if err := modules.RegisterModule(ctx, proxygridModule); err != nil {
		log.Errorf("Failed to register Proxy Grid module: %v", err)
	}
...
// and security headers to every response.
//
// Returns:
//   - gin.HandlerFunc: The CORS and security middleware handler
func corsMiddleware(allowedOrigins []string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// In production, tighten this list via configuration
...
// matchOrigin performs simple wildcard matching for origin validation
...
func AuthMiddleware(manager *sdkaccess.Manager) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if providers := manager.Providers(); len(providers) == 0 {
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"error": "No authentication providers configured"})
			return
		}
