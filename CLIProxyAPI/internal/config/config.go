...
type Config struct {
	...
	// CORS config controls which origins may call the HTTP API from browsers.
	...
	// Database holds database connection pool configuration.
...
type DatabaseConfig struct {
	...
	MaxConns int `yaml:"max-conns" json:"max-conns"`
	// MinConns is the minimum number of connections in the pool (default: 5).
	MinConns int `yaml:"min-conns" json:"min-conns"`
	...
	// MaxConnIdleTime is the maximum idle time of a connection (default: 30m).
	MaxConnIdleTime string `yaml:"max-conn-idle-time" json:"max-conn-idle-time"`
	// HealthCheck is the health check interval (default: 1m).
	HealthCheck string `yaml:"health-check" json:"health-check"`
}
...
type RateLimitConfig struct {
	...
	// RequestsPerMinute is the number of requests allowed per minute per client.
...
func LoadConfigOptional(configFile string, optional bool) (*Config, error) {
	...
	// Set default database pool config
	cfg.Database.MaxConns = 20
	cfg.Database.MinConns = 5
	cfg.Database.MaxConnLifetime = "1h"
	cfg.Database.MaxConnIdleTime = "30m"
	cfg.Database.HealthCheck = "1m"
	// Set default rate limit config (disabled by default for backward compatibility)
