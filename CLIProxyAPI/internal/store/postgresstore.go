...
type PostgresStoreConfig struct {
	...
	AuthTable   string
	SpoolDir    string
	// Pool config for connection pooling (optional)
	PoolConfig *db.PoolConfig
	// Replica DSNs for read replicas (optional)
	Replicas []string
}
...
// Uses pgxpool for efficient connection pooling and supports read replicas.
...
// Uses connection pooling via pgxpool for optimal performance.
func NewPostgresStore(ctx context.Context, cfg PostgresStoreConfig) (*PostgresStore, error) {
	...
	// Initialize the new database layer with connection pooling
	poolCfg := db.DefaultPoolConfig()
	if cfg.PoolConfig != nil {
		poolCfg = *cfg.PoolConfig
	}

	clusterCfg := db.ClusterConfig{
		Primary:  cfg.DSN,
	...
	repo, err := db.NewRepo(ctx, clusterCfg)
	if err != nil {
		return nil, fmt.Errorf("postgres store: initialize repo: %w", err)
	}
	store.repo = repo
	store.pool = repo.Cluster().Primary()

	// Also maintain sql.DB for backward compatibility
...
// GetPool returns the underlying pgxpool for direct access.
func (s *PostgresStore) GetPool() *pgxpool.Pool {
	return s.pool
}

// GetRepo returns the underlying database repository.
func (s *PostgresStore) GetRepo() *db.Repo {
	return s.repo
}
